...
class DummyTestCase(TestCase): #тесты на логику
    ...
    def test_bil_takoi_paren(self): #проверка на существование парня в базе и обновление его баллов
        name = 'Арсений'
        is_present = Tasks.objects.filter(title=name).exists()
        self.assertTrue(is_present == 1)
        postupok = str(Tasks.objects.get(title=name).size + ' ' + 'Принес кофе')
        Tasks.objects.filter(title=name).update(score=F("score") + 4, size = postupok)
        self.assertTrue('Уступил место Принес кофе', Tasks.objects.get(title=name).size)
        self.assertTrue(5, Tasks.objects.get(title=name).score)

    def test_nov_paren(self): #проверка на существование парня в базе и его добавление
        name = 'Кирилл'
        is_present = Tasks.objects.filter(title=name).exists()
        self.assertTrue(is_present == 0)
        Tasks.objects.create(title=name, boy_age=18, score=0, size='')
        self.assertTrue(name, Tasks.objects.get(id=4))
